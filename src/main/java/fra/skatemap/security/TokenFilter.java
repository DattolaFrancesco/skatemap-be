package fra.skatemap.security;

import fra.skatemap.entities.User;
import fra.skatemap.exceptions.NotAuthorized;
...
public class TokenFilter extends OncePerRequestFilter {
    ...
    protected boolean shouldNotFilter(HttpServletRequest request) throws ServletException {
        ...
        return matcher.match("/auth/**", path)
                ...
                || matcher.match("/actuator/**", path) // railway
                || matcher.match("/donations/**", path)
                || matcher.match("/bot/get/status/**", path)
                || matcher.match("/bot/ask", path);
